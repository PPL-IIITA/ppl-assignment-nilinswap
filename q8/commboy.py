	...
	def giftallocator(self,baske):#here baske is basket type
		...
		if self.typ=='m':#if he is miser type
			if not i:
				...
				lis=baske.sortasperprice()			#sort basket as per low to high price
				diffgiftbaslist=baske.threeofeach()
				self.giftbag+=diffgiftbaslist
				self.lgiftv=diffgiftbaslist[1].value
				self.mnyspt=mnyspt=diffgiftbaslist[0].price+diffgiftbaslist[1].price+diffgiftbaslist[2].price
				giftin=0
